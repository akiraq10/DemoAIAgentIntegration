...
public class FeatureGeneratorWithGemini {
    ...
    public static String generateFeatureFile(String testScenario,String featureName) throws IOException {
...
//                "Generate a Cucumber BDD feature file with @tag without Explanation of choices and improvements AND note in BDD feature for the following requirement:\n" + requirement)));
        prompt.put("contents", new JSONObject().put("parts", new JSONObject().put("text",
                "Generate a Cucumber BDD feature file without Explanation of choices and improvements AND note in Gherkin syntax with the feature name '" + featureName +
                        ". Ensure proper structure with '@tag',`Feature`, `Scenario`, `Given`, `When`, and `Then` steps. "+
                        "following requirement: " + testScenario )));
    ...
    public static void main(String[] args) {
        try {
            String featureText = generateFeatureFile("User should be able to log in to https://qavn.asia:444 with valid credentials.", "Login");
